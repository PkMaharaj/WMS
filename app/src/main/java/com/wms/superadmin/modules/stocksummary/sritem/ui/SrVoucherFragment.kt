...
import com.wms.superadmin.databinding.FragmentSrvoucherBinding
...
class SrVoucherFragment : BaseFragment<FragmentSrvoucherBinding>(R.layout.fragment_srvoucher),
    ...
    override fun onInitialized(): Unit {
        ...
        binding.itemName.text="Item: "+viewModel.navArguments?.getString(IntentParams.ITEM_NAME,"")
        mStockReportAdapter = StockReportAdapter(viewModel.stockReportList.value?:mutableListOf(), ScreenNames.SR_VOUCHERWISE)
                ...
                fun getStockReport(){
                    mStockReportRequest.screenName= ScreenNames.SR_VOUCHERWISE
                    mStockReportRequest.fromDate=mActivity.binding.txtfromdate.text.toString()
                    mStockReportRequest.toDate=mActivity.binding.txtTodate.text.toString()
                    viewModel.onClickOnCreate(mStockReportRequest)
                }
                override fun onDetach() {
                    super.onDetach()
                    mActivity.binding.txtfromdate.text=mActivity.mFromDate
                    mActivity.binding.txtTodate.text=mActivity.mToDate
                  // mActivity.setScreenAction(false,false,false,false,false)
                    ...
                    viewModel.stockReportListLiveData.observe(requireActivity()) {
                        if(it is SuccessResponse) {
                            ...
                            if(it.data.StockReportitemList?.isNotEmpty()!!){
                                ...
                                binding.txtBranch.text=if(mStockReportRequest.ismonthcheckbox!!) ScreenNames.SR_MONTHWISE else ScreenNames.SR_VOUCHERWISE
    ...
    companion object {
        const val TAG: String = "SR_VOUCHER_FRAGMENT"
        fun getInstance(bundle: Bundle?, activity: StockreportActivity,prious:Fragment): SrVoucherFragment {
            val fragment = SrVoucherFragment()
    ...
    override fun <T> onActionChange(`object`: T) {
        if(`object` is Boolean){
            if(`object`)
                Log.e("ActionChange","VoucherWise:")
